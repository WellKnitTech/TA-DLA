...
def scan_file_for_patterns(filepath: str, logger: Optional[logging.Logger] = None) -> List[Dict]:
    """
...
def scan_directory_for_pii_phi_pci(
    ...
    Args:
        ...
        max_workers: Number of parallel workers (default: 4 or os.cpu_count()).
        batch_size: Number of files to process per batch (default: 32).
        csv_output_path: Path to CSV file for streaming findings (recommended for large data sets).
    Returns:
    ...
    for i in range(0, len(all_files), batch_size):
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            future_to_file = {executor.submit(scan_file_for_patterns, fpath, logger): fpath for fpath in batch}
